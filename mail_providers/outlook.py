...
def fetch_otp_via_graph(
    ...
    # Graph 单次往返：token 刷新 ~6s + 首次请求 ~5.5s ≈ 11.5s。
    # deadline 低于 30s 时几乎必然超时（轮询还没开始就结束了），
    # 这里强制下限，避免调用方误设过小 timeout 导致必失败。
    if deadline - time.time() < 30:
        logger.warning(
            f"[outlook-graph] deadline 过短({deadline - time.time():.0f}s)，"
            f"强制提高到 30s（token 刷新+首轮扫描约需 12s）"
        )
